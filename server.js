...
app.post("/api/export.pdf-email", adminOnly, async (req, res) => {
  ...
  try {
    ...
    if (!transporter) return res.status(503).json({ error: "Email sending is not configured. Set SMTP_HOST, SMTP_USER, SMTP_PASS, and SMTP_FROM in .env, then restart the server." });
    await transporter.sendMail({
      ...
      attachments: [
        ...
        { filename: filename.replace(/\.pdf$/i, ".csv"), content: "\uFEFF" + csv, contentType: "text/csv" }
      ]
    });
    res.setHeader("Content-Type", "application/pdf");
