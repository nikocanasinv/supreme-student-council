...
if (!process.env.BREVO_API_KEY) {
  return res.status(503).json({
    error: "Email sending is not configured. Add BREVO_API_KEY in Render Environment Variables."
  });
}

if (!process.env.BREVO_FROM_EMAIL) {
  return res.status(503).json({
    error: "Email sender is not configured. Add BREVO_FROM_EMAIL in Render Environment Variables."
  });
}
...
try {
  ...
  const result = await brevo.transactionalEmails.sendTransacEmail({
    sender: {
      ...
      name: process.env.BREVO_FROM_NAME || "Council Inventory System"
    },
    ...
    textContent:
      `Attached is the Council Inventory Report generated by ${req.session.user.full_name}.`,
    ...
    attachment: [
      ...
      {
        name: filename.replace(/\.pdf$/i, ".csv"),
        content: Buffer.from("\uFEFF" + csv, "utf8").toString("base64")
      }
    ]
  });

  console.log("BREVO REPORT EMAIL SENT:", result.messageId);

} catch (emailError) {
  console.error("BREVO EMAIL ERROR:", emailError);

  return res.status(502).json({
    error: emailError?.message || "Brevo rejected the email."
  });
}
    res.setHeader("Content-Type", "application/pdf");
